...
const GET = (req, res, next) => {
  try {
    ...
    if (userId) {
      let [user] = read("users").filter((user) => {
...
const LOGIN = (req, res, next) => {
  try {
    ...
    let user = users.find(
      (user) =>
        (user.username == username || user.email == username) && user.password == sha256(password)
    );
...
const REGISTER = (req, res, next) => {
  try {
    ...
    if (user) return next(new AuthorizationError(401, "this username or email exits"));
